...
const buildLobby = (
	...
	id: Math.random(),
	slots: { occupied: 0, max: 1 },
	messages: [],
	created: Date.now(),
});

const stRule = buildOrRule([
	buildTermRule("map", /sheep.*tag/i),
	buildTermRule("name", /sheep.*tag/i),
]);
...
describe("ruleToFilter", () => {
	...
	it(
		"sheep tag",
		testLobbies(ruleToFilter(stRule), [true, false, false, true, false]),
	);
});
