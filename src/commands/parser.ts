...
const simplify = (rule: Rule): RootRule => {
	if (isGroupRule(rule)) {
		...
		for (const childRule of rule.rules) simplify(childRule);
...
export const parser = (
	string: string,
): { filter: Rule; options?: { message?: string } } => {
	...
	for (const term of terms)
		switch (term.type) {
			...
			case "term": {
				if (rule.type === "term" && rule.value === "") continue;
