...
export const process = ( rule: Rule, lobby: Lobby ): boolean => {
	...
	switch ( rule.type ) {
		...
		case "or": return rule.rules.some( rule => process( rule, lobby ) );
		case "and": return rule.rules.every( rule => process( rule, lobby ) );
		case "option": throw new Error( "term in rule AST" );
...
export const ruleToFilter = ( rule: Rule ): ( ( lobby: Lobby ) => boolean ) =>
	( lobby: Lobby ): boolean => process( rule, lobby );
